...
* **default_value** (string) Optional, a default value that the object assigns the parameter if the parameter is not given in the SST configuration file. If a parameter is required, make this NULL or omit it. **Note: this does not add a default value to the component. Rather, it is used only to communicate to the user what will happen if they don't provide a value. The author of a component must ensure defaults are set via `params.find<T>()`. These two default values are not checked against each other. **
